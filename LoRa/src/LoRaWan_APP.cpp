...
/*!
	Callback function for when a join request occurs. 
*/
void (*JoinRequestCallback)();
/*!
	Callback function for when a join is successful.
*/
void (*JoinSuccessfulCallback)();
/*!
	Passed in Join() function, determines the sleep amount in between join attempts.
...
{
	Serial.println("INSIDE MlmeConfirm");
	switch( mlmeConfirm->MlmeRequest )
	{
		case MLME_JOIN:
		{
			if( mlmeConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK )
			{
				...
				printf("joined\r\n");
				// Call callback.
				// About to enter callback.
				Serial.println("Entering CALLBACK");
				JoinSuccessfulCallback();
			...
			{
				// Call the join increment counter.
				JoinRequestCallback();
				uint32_t rejoin_delay  = joinDelay;
				printf("join failed. Attempt Join again in ");
				Serial.println( rejoin_delay);
				printf("MS.");
...
void lwan_dev_params_update( void )
{
	if(loraWanRegion == LORAMAC_REGION_EU868 ){
		Serial.println("LORA REGION EU868 - adding the channels.");
...
void LoRaWanClass::join(void (*incremJoinCountCallback)(), 
	void (*joinSuccessfulCallback)(), uint32_t joinDelayMS ){
	if( overTheAirActivation )
	{
		Serial.print("joining...");
		MlmeReq_t mlmeReq;
		
		Serial.println("Setting my callbacks.");
		JoinRequestCallback = incremJoinCountCallback;
		JoinSuccessfulCallback = joinSuccessfulCallback;
		
		joinDelay = joinDelayMS;
		mlmeReq.Type = MLME_JOIN;
		...
		mlmeReq.Req.Join.AppKey = appKey;
		mlmeReq.Req.Join.NbTrials = 1;
		Serial.println("About to ender LoRaMaclmerequest");
		if( LoRaMacMlmeRequest( &mlmeReq ) == LORAMAC_STATUS_OK )
		{
			Serial.println("About to ender LoRaMaclmerequest done, sleeping.");
			deviceState = DEVICE_STATE_SLEEP;
		}
		else
		{
			Serial.println("About to ender LoRaMaclmerequest done, cycling.");
			deviceState = DEVICE_STATE_CYCLE;
		}
	}
	else
...
//if saved net info is OK in lorawan mode, skip join.
	if(checkNetInfo()&&modeLoraWan){
		...
		Serial.println("About to init. DONE");
		getNetInfo();
		Serial.println("About to getNetInfo DONE");
		if(passthroughMode==false){
