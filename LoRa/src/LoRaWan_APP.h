...
public:
  ...
  // Used to notify Main.cpp that join has failed, and that increment counter sould +1.
  void join(void (*incremJoinCountCallback)(), 
  // Used to notify Main.cpp that join succeed, and that this should be saved to memory.
	void (*joinSuccessfulCallback)(),
  // Used to determine how long the join delay should last (longer if never join before, presumably over an ocean if not).
  uint32_t joinDelay );
  void send();
