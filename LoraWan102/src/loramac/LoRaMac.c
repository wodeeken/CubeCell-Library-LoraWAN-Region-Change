...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    //Serial.println("LoRaMacConfirmQueueIsFull");
    if( LoRaMacConfirmQueueIsFull( ) == true )
    {
       // Serial.println("LoRaMacConfirmQueueIsFull return");
        return LORAMAC_STATUS_BUSY;
    }
    ...
    switch ( mlmeRequest->Type ) {
        case MLME_JOIN: {
           ...
           // Serial.println("RegionVerify true");
            ...
            //Serial.println("LoRaMacConfirmQueueAdd dome");
            ...
            ResetMacParameters( );
           // Serial.println("Reset mac params");
            altDr.NbTrials = JoinRequestTrials + 1;
#ifdef CONFIG_LINKWAN
...
#endif
            LoRaMacParams.ChannelsDatarate = RegionAlternateDr( LoRaMacRegion, &altDr );
         //   Serial.println("RegionAlternateDr done");
#ifdef CONFIG_LORA_VERIFY
            if (g_lora_debug == true)
                PRINTF_RAW("MacHdr major:%d rfu:%d mtype:%d\r\n", macHdr.Bits.Major, macHdr.Bits.RFU, macHdr.Bits.MType);
#endif
        ...
        case MLME_LINK_CHECK: {
          //  Serial.println("LoRaMacConfirmQueueAdd");
            // Apply the request
            LoRaMacFlags.Bits.MlmeReq = 1;
            queueElement.Request = mlmeRequest->Type;
            ...
            queueElement.RestrictCommonReadyToHandle = false;
            LoRaMacConfirmQueueAdd( &queueElement );
            //Serial.println("LoRaMacConfirmQueueAdd done");
            // LoRaMac will send this command piggy-pack
            status = AddMacCommand( MOTE_MAC_LINK_CHECK_REQ, 0, 0 );

           // Serial.println("AddMacCommand done");
            break;
        }
        case MLME_TXCW: {
            // Apply the request
            ...
            queueElement.Status = LORAMAC_EVENT_INFO_STATUS_ERROR;
            queueElement.RestrictCommonReadyToHandle = false;
         //   Serial.println("MLME_TXCW LoRaMacConfirmQueueAdd");
            LoRaMacConfirmQueueAdd( &queueElement );
           // Serial.println("MLME_TXCW LoRaMacConfirmQueueAdd done");
            //Serial.println("MLME_TXCW SetTxContinuousWave");
            status = SetTxContinuousWave( mlmeRequest->Req.TxCw.Timeout );
            //Serial.println("MLME_TXCW SetTxContinuousWave done");
            break;
        }
        case MLME_TXCW_1:
        {
          //  Serial.println("MLME_TXCW_1 ");
            // Apply the request
            LoRaMacFlags.Bits.MlmeReq = 1;
            queueElement.Request = mlmeRequest->Type;
            queueElement.Status = LORAMAC_EVENT_INFO_STATUS_ERROR;
           ...
           // Serial.println("MLME_TXCW_1 done");
            break;
        }
        case MLME_DEVICE_TIME:
        {
           // Serial.println("MLME_DEVICE_TIME ");
            // Apply the request
            LoRaMacFlags.Bits.MlmeReq = 1;
            queueElement.Request = mlmeRequest->Type;
            queueElement.Status = LORAMAC_EVENT_INFO_STATUS_ERROR;
           ...
           // Serial.println("MLME_DEVICE_TIME done");
            break;
        }
        case MLME_PING_SLOT_INFO:
        {
           // Serial.println("MLME_PING_SLOT_INFO");
           ...
           /// Serial.println("MLME_PING_SLOT_INFO done");
            break;
        }
